    ...
    <section id="process" className="py-12">
      <div className="container mx-auto px-4">
        <div className="flex justify-between items-end gap-6 mb-7 flex-wrap">
          <div>
            <h2 className="text-2xl font-serif font-semibold tracking-wide">Accompagnement</h2>
          </div>
          ...
          <div className="border-l border-border pl-5 space-y-5">
            {steps.map((step) => (
              <div key={step.number} className="relative">
                <div className="absolute -left-[2.4rem] top-0.5 w-7 h-7 rounded-full border border-accent bg-white flex items-center justify-center text-xs text-accent font-medium">
                  {step.number}
                </div>
                <div className="text-sm font-serif font-medium mb-1 text-foreground">
                  {step.title}
                </div>
